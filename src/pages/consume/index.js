...
export default class Index extends wepy.page {
    ...
    getMoney() {
        if (this.couponForConsume) {
            return (
                +this.selectedProd.money - +this.couponForConsume.money
            ).toFixed();
        } else {
    ...
    methods = {
        ...
        chooseCoupon(idx) {
            if (this.useCoupon.checked) {
                wepy.navigateTo({
                    url: `/pages/coupon/index?from=consume&maxMoney=${
                        this.selectedProd.money
                    }`
                });
            }
        },
    ...
    reqMXZ030004() {
        ...
        MXZ030004Service({
            ...
            ({ data: { resultCode, resultMsg, data } }) => {
                this.requestIng = 0;
                if (resultCode === "0000") {
    ...
    wxPay() {
        ...
        MXZ050001Service({
            ...
            couponMoney: this.couponForConsume
                ? this.couponForConsume.money
                : "", // 优惠券金额
            codeId: "" + id,
            money: this.getMoney(),
            attach: "wxpay"
